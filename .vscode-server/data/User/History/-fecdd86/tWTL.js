...
const ShowEquipe = () => {
  ...
  useEffect(() => {
    const fetchEquipeData = async () => {
      try {
        const response = await fetch(`https://api.ligue1.live/api/equipe?id_saison=269&id_equipe=${id}`);
  ...
  return (
    <div className="containershowequipe">
      ...
      <h1 className="teamNameShowEquipe">{equipeData.teamName}</h1>
        ...
        {equipeData.players && equipeData.players.map((player) => (
          <Link to={`/ShowPlayer/${player.playerID}`} key={player.playerID} className="playerLink">
            <div className="player">
              ...
              <p className="playerNameShowEquipe">{player.fullName}</p>
