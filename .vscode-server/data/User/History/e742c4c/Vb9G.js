  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        ...
        if (goalScorersData && assistProvidersData.items && assistProvidersData.items.season && assistProvidersData.items.season.players) {
          setGoalScorers(assistProvidersData.items.season.players);
        } else {
          console.error('Error: data not found in response');
        }
        const assistProvidersResponse = await fetch('https://api.ligue1.live/api/stat?id_saison=269&id_event=');
        const assistProvidersData = await assistProvidersResponse.json();
        if (goalScorersData && assistProvidersData.items && assistProvidersData.items.season && assistProvidersData.items.season.players) {
          setGoalScorers(assistProvidersData.items.season.players);
        } else {
