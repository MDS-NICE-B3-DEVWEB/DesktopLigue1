...
class StatJoueurController extends Controller
{
    public function joueurStatApi(Request $request)
    {
        try {
            $client = new Client();
        
            $id_saison = $request->id_saison;
            $id_event = $request->_id;
            
            // Faites l'appel à l'API externe en incluant les paramètres de la requête
            $response = $client->get("https://api.statorium.com/api/v1/topplayers/{$id_saison}/?apikey=12238dbb87bf7233b8ab70b36192a3ec&event_id={$event_id}&limit=5");
            ...
            return response()->json([
                'status_code' => 200, 
                'success' => true,
                'message' => 'Liste des meilleurs joueurs',
                'items' => $dataArray,
            ]);
        } catch (\Exception $e) {
